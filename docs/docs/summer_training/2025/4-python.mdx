...
[列表推导式 (List Comprehensions)](https://docs.python.org/3.13/tutorial/datastructures.html#list-comprehensions) 是一种实用的构造列表的方式. 假设我们现在需要用到一个包含 $9^2$ 以内平方数的列表, 可以以以下代码构造:
...
这是一个颇为有趣的语法糖, 并且由于这种方式不存在列表逐个扩容的过程, 在面对大型数组的构造时这种方法效率还会更高. 更多例子:

```python
# squares, but only contains even numbers:
even_squares = [i ** 2 for i in range(10) if i % 2 == 0]
```

```python
# A point set
points = [(x, y) for i in range(10) for j in range(5) if x != y]
```
