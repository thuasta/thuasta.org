...
function Todo(props) {
  return (
    <li>
      {props.person} should {props.todo}
    </li>
  );
}
...
function ShoppingList() {
  ...
  function Next() {
    if (targetId === 3) setTargetId(1);
    else setTargetId(targetId + 1);
  }
...
- 对于数组、对象等复杂类型，React 将检查引用的地址是否相等。这种行为可以优化性能，但有时会造成意外的组件不更新，请注意这一点
...
Pages Router 是 Next.js 最早的路由方案，它将 pages 目录下的所有文件都视作路由文件，并且文件名就是路由的路径。例如，我们有一个 pages 目录，其结构如下：
